// THIRD-PARTY  MIT-LICENSE: http://rem.mit-license.org/
...
var EventSource = function (url) {
  var eventsource = this,  
      ...
      lastEventId = null,
      index = 0,  // index into responseText
      // event type and data
    ...
    try { // force hiding of the error message... insane?
      ...
      xhr.onreadystatechange = function () {
        ...
        if (this.readyState == 3 || (this.readyState == 4 && this.status == 200)) {
          ...
          var text = responseText.substr(index);
          //console.log("text(" + text.length + ")=" + text);

          var parts = text.split("\n");
          var remainder = parts.pop();
          index = responseText.length - remainder.length;
          ...
          if (this.readyState == 4) { // and some other status
            ...
            eventsource.dispatchEvent('error', { type: 'error', data: 'connection lost' });
            pollAgain(interval);
...
EventSource.prototype = {
  ...
  dispatchEvent: function (type, event) {
    //console.log("dispatching eventtype " + type);
    //console.log(event);
